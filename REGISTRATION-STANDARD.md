...
**⚠ `ID-3` nói "tập chứng thực", không nói `personhood_level`.** Nguồn duy nhất của hình dạng tập
đó là `PhoenixKey-Specs/PhoenixKey-Knowme-Math.md` §4.2 *"Tư-cách-người là một TẬP mệnh-đề, KHÔNG
phải một thang số"* @ `ca13914` (2026-09-12). Mục đó định nghĩa ba tầng — `Attestation` (có `kind` và
`family`) → `PersonhoodView(d, τ)` → cổng năng lực `can(d, c, τ)` — kèm phản chứng vì sao một trường
`*_level` mang kiểu số là **sai kiểu** (§4.2.1). Tệp này **không chép** danh sách `kind`/`family`:
bên tích hợp tra §4.2.4 ở nguồn. Bên cần một năng lực thì hỏi `can(...)` cho năng lực đó, không so
một con số.

<details><summary>Lịch sử — hai bản trước của đoạn này đã hết đúng</summary>

Bản 2026-08-30 đo `personhood` **0 lần** trong `PhoenixKey-Specs/` — đúng lúc đo, hết đúng từ
2026-09-12 khi §4.2 vào nhánh chính. Bản đó còn chép tập
`{ did-chain, hardware-rooted-key, person-in-jurisdiction(…) }` từ một thư trao đổi giữa hai bên
(2026-08-14); §4.2 đã thay tập đó bằng bảng `kind`/`family`. Bản chép không có đường về nguồn nên
không gì báo nó đã chết — đó là lý do đoạn trên chỉ trỏ.
</details>
...
- **`TK-1` KHÔNG hỏi về CARP**, dù vế "dùng CARP" nằm trong ba điều kiện §10 trích ở đầu tài liệu.
  ...
  `MagicLampEco/MAGIC/Specs/MagicLamp-Tripletoken-Feat-(Vi).md` @ `4a08407e`: khối *"Ba dòng vào
  MAGIC + một dòng ra"* (`:89-92` tại sha đó) cho thấy **ba dòng vào**, và dòng thứ nhất là
  `LAMP → MAGIC` qua InstantGen/ScheduleGen — **không chạm CARP**. §9 *"Hợp đồng tín dụng MAGIC
  đa-nguồn"* (`:702-707`) viết thẳng app *"chọn nguồn khoá"*; Nguồn A = nắm LAMP; Nguồn B = khoá
  CARP. (Neo theo tên mục: con trỏ cũ `MAGIC/SPEC/…:65-67`, `:376-379` chết hai lần cùng lúc — thư
  mục đổi tên `SPEC/` → `Specs/`, và số dòng trôi hơn 20 dòng ở khối đầu, hơn 320 dòng ở §9.)
...
| Tính duy nhất một-người-một-DID | `did:phoenix` mới ép ở **mức chuỗi DID**. **Mức người và cả mức thiết bị đều chưa ép** — dẫn chứng ở §2.1. Chủ dự án đã chốt "một người = một DID" là **yêu cầu cứng** (2026-08-06). Hình dạng tầng personhood: `PhoenixKey-Knowme-Math.md` §4.2 @ `ca13914` — một **tập mệnh đề** (`Attestation` có `kind` + `family`), không phải một thang số; cổng năng lực `can(d, c, τ)` hỏng-đóng, không nhánh mặc định. Hai bất biến ở đó quyết trực tiếp dòng này: **I-KNOW-17** — năng lực nhạy đòi mệnh đề từ **≥ 2 family**, đếm family chứ không đếm mệnh đề; và **I-KNOW-19** — tạo DID **không** đòi mệnh đề nào, năng lực mới bị khoá. Cũng ở đó (§4.2.4): mở khoá bằng sinh trắc trên máy thuộc family `PossessionDevice`, **không** phải bằng chứng thân thể, và mẫu sinh trắc **không so chéo máy**. Hệ quả cho dòng này: chưa family nào ở §4.2 phát biểu "một người chỉ có một DID trên toàn cầu", và §4.2.7 ghi rõ family `Embodiment` hôm nay **chưa nhà nào cấp được**. ⚠ Bản trước của ô này chép tập `{ did-chain, hardware-rooted-key, person-in-jurisdiction(…) }` từ một thư trao đổi 2026-08-14 — tập đó **đã bị §4.2 thay**, đừng chép lại. | Đừng dựng cơ chế chống Sybil dựa trên "hai DID phân biệt" — một máy ký chéo cho chính nó bằng script được. Rào kinh tế đỡ được **một phần**, không phải tất cả — đọc kỹ ranh giới này. Ràng buộc: phần thưởng phát ra mỗi epoch phải bị chặn trên bởi một phần **nhỏ hơn 1** của lượng MAGIC thực bị tiêu cùng epoch, phần dư về Treasury. Nó làm kẻ **tự tạo cầu giả** để farm luôn lỗ, kể cả khi tạo được vô hạn DID. Nhưng nó **im lặng** trước đường thứ hai: **không đốt gì cả, chỉ khai CUNG để lấy phần của lượng người khác đã đốt** — đường đó có lợi nhuận dương với mọi tỉ phần và mọi hệ số, và chỉ chặn được bằng biên nhận do bên **không hưởng lợi** ký, thách thức có phát hiện thật, và cổng DID cho node. Chứng minh và phản ví dụ: [`bench/DOI-CHIEU.md`](bench/DOI-CHIEU.md) §1. |
