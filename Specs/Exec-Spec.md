...
| **M1** | **Datum + redeemer test** (TECH §2): encode/decode `PlatformEntry` (đủ `PLATFORM_ENTRY_FIELDS` trường, Constr order) + `PlatformStatus` (0/1/2) round-trip Aiken↔off-chain. Invariant: entry register-status==Active. | M0 | datum round-trip pass; **register status≠Active → reject** (R-WF); **platform_id≠NFT name → reject** (R-NAME). |
