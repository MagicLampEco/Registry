...
>
> **v2.1 thêm ĐÚNG ba mã, cùng một bất biến ở ba cửa**: `R-ADDR` (`registry_beacon.ak:115`) ·
> `U-ADDR` (`registry.ak:197`) · `M-ADDR` (`registry.ak:294`) — ô hồ sơ phải ở địa chỉ **enterprise**,
...
> `registry_beacon.ak:90-197`) + **1** `S-*` (chung hai nhánh spend — `registry.ak:177`) + **15** `U-*`
> (`registry.ak:182-265`) + **14** `M-*` (`registry.ak:272-332`) = **44**. Phát biểu hình thức của
...
> SÁU trường (`offchain/src/registrationBuilder.ts:246-254`), datum mã hoá đúng MƯỜI MỘT trường
> (`offchain/src/registryDatum.ts:127-137`), và `planMigrateEntry` đã có
> (`offchain/src/registrationBuilder.ts:893`). Hai con số cũ trong §6 chỉ là chữ sót, đã sửa tại chỗ.
...
`registry.ak:161`) → ra `registry_hash` → truyền vào `registry_beacon(authority, registry_hash)`
(`registry_beacon.ak:87`). Đây là chiều **đảo** so với v1; lý do và cái giá phải trả ở §5.
...
một validator `registry` KHÁC (`registry.ak:270`). Nhánh thứ hai có vì tham số validator đổi thì script
hash đổi, tức địa chỉ đổi — không hồi tố được, phải dời từng hồ sơ (`registry.ak:145-148`).
...
`MigrateEntry` constr 1 — `platform.ak:130-137`). Mọi nhánh khác else-fail.

> **Nhánh di trú CỐ Ý KHÔNG áp `U-TERMINAL`** (`registry.ak:135-137`; ràng buộc `entry_in.status !=
> Retired` chỉ nằm ở nhánh Update — `registry.ak:204`). Lý do: nếu áp cả hai nhánh thì mỗi lần xoay quyền
...
**hai** tham số (`registry_beacon.ak:87`). `registry_hash` là thứ v1 không có; nhờ nó cửa đúc ép được
địa chỉ đích ngay lúc sinh hồ sơ (R-OUT-1). Kiểm **mười bốn** ràng buộc, cộng BURN cấm (else fail —
`registry_beacon.ak:202-204`). Thứ tự dưới đây là thứ tự chạy thật trong mã:

```
R-SIG     authority ký:  list.has(tx.extra_signatories, registry_authority)     (`:90`)
...
R-MINT-1  mint ĐÚNG 1 token policy này, qty +1:                                 (`:93-96`)
...
R-MINT-2  least-authority (vá lần 2 LỖ #F5):  list.length(assets.policies(tx.mint)) == 1   (`:99`)
...
R-OUT-1   ĐÚNG 1 output mang NFT đó (SELF-REF), ở ĐÚNG Script(registry_hash) + có datum
          (`registry_beacon.ak:104-106`, datum đọc ở :117-118):
...
R-ADDR    ô hồ sơ ra ở địa chỉ ENTERPRISE của registry (v2.1 — `:115`):
            util.has_no_stake_part(entry_out.address)     // stake_credential == None (`util.ak:70-72`)
          ...
          ⚠ ĐÍNH CHÍNH (đo 2026-09-01) — bản trước của dòng này, và chú thích trong `util.ak`, ghi rằng
            ...
            Kho này KHÔNG có call site `utxosAt` nào; `scripts/config.ts:126` chỉ DỰNG địa chỉ, không ĐỌC
          ...
          ⚠ VÀ R-ADDR KHÔNG ĐỦ để hai đường đọc luôn đồng ý (audit đối kháng 2026-09-01, có PoC chạy
            ...
            ⇒ Kết luận cho bên đọc: đọc sổ bằng `utxosAt` là đường KHÔNG LÀNH. Đường lành duy nhất là
              ...
              đang làm.
          > Đánh đổi có chủ ý (`util.ak:67-69`): ghim enterprise là BỎ quyền uỷ thác phần ADA khoá trong
...
R-NAME    entry.platform_id == platform_id   // datum khai đúng tên NFT đã mint (`:121`)
          ⇒ platform_id trong datum == asset name → entry không thể nói dối định danh.

R-POLICY  entry.beacon_policy == policy_id                              (`:125`)
          ⇒ `beacon_policy` là LỜI TỰ KHAI trong datum, và `registry.ak` đọc thẳng lời khai đó
            (`registry.ak:172`). Không có dòng này thì bên spend bị dắt mũi bằng một policy tự chế.
...
R-VER     entry.spec_version == platform.spec_version_v2   // = 2 (`platform.ak:58`, ép ở `:128`)
          ⇒ đăng ký mới phải theo lược đồ hiện hành. Đường tăng phiên bản DUY NHẤT là `MigrateEntry`
            (M-VER); nhánh Update khoá cứng (U-VER).

R-WF      platform.entry_well_formed(entry)  — v2, nguồn `platform.ak:209-213`:
          ...
          > `governance_ref` bắt buộc cho CẢ HAI hạng, kể cả hạng không kho (`platform.ak:205-208`).
...
R-GOVLIVE util.governance_consented(tx, entry.governance_ref)            (`:165`)
          ⇒ cổng quản trị phải CHẠY THẬT ngay trong chính tx đăng ký (chi tiêu một input ở
            Script(governance_ref), hoặc một withdrawal từ đó — `util.ak:230-242`). Ép 28 byte ở R-WF
            chỉ là guard CÚ PHÁP: 28 byte đúng độ dài vẫn có thể là hash của script không tồn tại, hoặc
            của script không có nhánh nào chạy được (`platform.ak:47-52`). Kiểm bằng thực thi — PoC
          ...
          > Vì sao ép ở ĐÂY: đăng ký là điểm DUY NHẤT trong vòng đời còn đòn bẩy. Sau khi lên sổ, MỌI
            ...
            ai gỡ được (`registry_beacon.ak:138-142`).
          > ⚠ **XUNG ĐỘT ĐÃ BIẾT, CỐ Ý KHÔNG VÁ: R-GOVLIVE ⟂ R-MINT-2** (`registry_beacon.ak:147-164`).
          ...
          > ⚠ Giới hạn kế thừa: `governance_consented` chứng minh "script ĐÃ CHẠY", KHÔNG chứng minh
            ...
            (`util.ak:215-224`). ⟹ an toàn của R-GOVLIVE/U-GOV/M-GOV = an toàn của validator quản trị
            mà chính platform khai.

R-VALUE   without_lovelace(entry_out.value) == from_asset(policy_id, platform_id, 1)   (`:169-173`)
...
R-EPOCH   entry.created_epoch == util.current_time_bucket(tx)            (`:181`)
          ...
          > ⚠ **TÊN TRƯỜNG NÓI SAI ĐƠN VỊ.** `created_epoch` KHÔNG phải epoch Cardano: giá trị là "ô 5
            ngày kể từ mốc Unix" (`posix_ms / 432_000_000` — `util.ak:128-136, 152`). Tại mốc Shelley
            hàm trả **3694**; biên ô lật sớm hơn biên epoch thật ~**251.091 s ≈ 2,9 ngày**; trên Preview
            (epoch 1 ngày) sai số là **5×**. Giữ tên vì đổi tên = đổi lược đồ (`platform.ak:114-119`).
          ...
          > ⚠ "Registry và Treasury cùng quy ước thời gian" — **CHƯA KIỂM**, đừng viết như đã kiểm. Bên
            ...
            trong test (`util.ak:143-148`). Mệnh đề đó chỉ đúng NẾU bên triển khai truyền đúng số ấy.

R-BIND    entry trỏ kho THẬT — CHỈ ÁP CHO HẠNG CÓ KHO (`:194-197`):
            or { !platform.shape_custodial(entry), custody_bound(tx, entry) }
          với `custody_bound` (`:72-85`): có một reference input mang ĐÚNG 1 NFT
          ...
          > **Bỏ R-BIND cho hạng KHÔNG KHO không hạ an toàn** (`registry_beacon.ak:190-193`): ba giá trị
          ...
          > ⚠ **ĐỌC ĐÚNG SỨC MẠNH.** Cả ba giá trị đều lấy từ CHÍNH datum người đăng ký khai, nên đây
            ...
            (`registry_beacon.ak:186-189`). Cổng thật lúc đăng ký là chữ ký authority. Bên định tuyến phí
...
BURN — CẤM (else fail — `:202-204`): beacon sống suốt vòng đời platform; retire = status trong datum (PK5).
```
...
`validator registry(registry_authority: ByteArray)` — **MỘT** tham số (`registry.ak:161`). v2 bỏ tham số
`beacon_policy`; nay nó đọc từ `entry_in.beacon_policy` (`registry.ak:172`). Nhờ vậy hash của validator
này tính được TRƯỚC và cửa đúc mới ép được địa chỉ đích (§5).

Hai action: `UpdateEntry` (constr 0) và `MigrateEntry` (constr 1) — `platform.ak:130-137`.

### 4.0 Ràng buộc CHUNG cho cả hai nhánh

```
S-GOVSELF entry_in.governance_ref != own_hash                            (`registry.ak:177`)
          ⇒ ô hồ sơ đang chi tiêu LUÔN nằm ở Script(own_hash), mà `governance_consented` quét MỌI
            tx.inputs tìm input ở Script(governance_ref) (`util.ak:231-232`). Nên một hồ sơ khai
...
### 4.1 `UpdateEntry` — 15 mã (`registry.ak:182-265`)

```
U-MINT-0  assets.is_zero(tx.mint)                                        (`:182`)
          ⇒ update KHÔNG đụng beacon supply (không mint, không burn).

U-SINGLE  ĐÚNG 1 ô hồ sơ vào + 1 ô ra, đếm theo SCRIPT HASH:             (`:185-188`)
...
U-ADDR    ô hồ sơ ra ở địa chỉ ENTERPRISE của chính registry (v2.1):     (`:197`)
...
U-TERMINAL entry_in.status != Retired                                    (`:204`)
          ⇒ Retired là trạng thái CUỐI của ĐƯỜNG UPDATE. Vòng đời một chiều: Active⇄Paused→Retired.
            ...
            áp mã này (`:135-137`). Xem §4.2.

U-ID      platform.identity_preserved(entry_in, entry_out)               (`:207`)
            platform_id ∧ instance_id ∧ custody_hash ∧ seed_policy ∧ beacon_policy ∧ created_epoch
            BẢO TOÀN — SÁU trường (`platform.ak:218-220`)
          ⇒ update KHÔNG đổi định danh — đổi = platform khác, phải đăng ký mới.

U-VER     entry_out.spec_version == entry_in.spec_version                (`:209`)
          ⇒ đổi phiên bản lược đồ phải đi đường MigrateEntry (M-VER). `spec_version` KHÔNG nằm trong
            nhóm identity vì nó bất biến ở đây nhưng TĂNG ở kia (`platform.ak:216-217`).

U-MUT     platform.mutable_fields_valid(entry_out)                       (`:211`)
...
U-GOVSELF-OUT entry_out.governance_ref != own_hash                       (`:217`)
          ⇒ NỬA CÒN LẠI của S-GOVSELF. Đợt trước chỉ chặn ở datum VÀO và ở cửa nộp; datum RA chỉ bị
            ...
            beacon NFT + min-ADA khoá vĩnh viễn ⇒ phá thẳng PK5 (`registry.ak:69-76`).

U-SHAPE   shape_custodial(entry_in) == shape_custodial(entry_out)        (`:219`)
          ⇒ không đổi hạng có-kho ↔ không-kho. Chuyển hạng = đổi bản chất dịch vụ ⇒ phải đăng ký mới.

U-NFT     beacon NFT bảo toàn ở vào & ra:                                (`:222-223`)
            has_one_nft(reg_in.value,  beacon_policy, entry_in.platform_id)
            has_one_nft(reg_out.value, beacon_policy, entry_out.platform_id)
          với `beacon_policy = entry_in.beacon_policy` (`:172`) — lấy từ datum, không phải tham số.
          ⇒ chỉ hồ sơ "thật" (mang beacon NFT) mới chi tiêu được; NFT không rời ô hồ sơ.

U-VALUE   util.value_not_drained(reg_in.value, reg_out.value)            (`:226`)
          ...
          ⇒ không rút bớt khỏi ô hồ sơ. Hôm nay sổ chưa giữ giá trị nên đây là ràng buộc dự phòng; bản
            nào đặt tiền cọc vào ô hồ sơ thì thiếu nó là lỗ rút tiền ngay (`util.ak:253-256`).
          > ⚠ GIỚI HẠN CỐ Ý GIỮ (`util.ak:258-262`): chỉ cho lovelace TĂNG ⇒ ADA ai đó gửi nhầm vào ô
...
U-SIG     authority ký, TRỪ ca hồi sinh thuần tuý:                       (`:237-243`)
...
U-GOV     việc KHÔNG ĐẢO NGƯỢC ĐƯỢC đòi đồng thuận quản trị:             (`:247-255`)
...
> mặt **cấu trúc** (`platform.ak:242-249` không nhắc tới nó). Ràng buộc **vòng đời** ép riêng qua
> U-TERMINAL + U-GOV + U-REVIVE.

**U-SINGLE đếm theo PAYMENT SCRIPT HASH** (`util.ak:84-90`) — KHÔNG full-address: chống
...
### 4.2 `MigrateEntry` — 14 mã (`registry.ak:272-332`)

Di trú đưa hồ sơ sang một validator `registry` KHÁC. Có nhánh này vì đổi tham số validator thì đổi
script hash, tức đổi địa chỉ — không hồi tố được (`registry.ak:145-148`).

```
M-MINT-0  assets.is_zero(tx.mint)                                        (`:272`)
M-SIG     list.has(tx.extra_signatories, registry_authority)             (`:275`)
          ⇒ KHÔNG có ngoại lệ kiểu U-REVIVE ở nhánh này.
M-GOV     governance_consented(tx, entry_in.governance_ref)              (`:278`)
          ⇒ luôn đòi, không điều kiện: di trú đưa hồ sơ ra khỏi quyền tài phán của validator này, nên
            platform phải đồng ý.

M-DEST    len(new_registry_hash) == 28  ∧  new_registry_hash != own_hash  (`:283-284`)
            count_inputs_at_script(tx.inputs, own_hash) == 1              (`:285`)
            count_outputs_at_script(tx.outputs, new_registry_hash) == 1   (`:286`)
          ...
          > ⚠ **ĐỌC ĐÚNG SỨC MẠNH: đây CHỈ là kiểm ĐỘ DÀI**, không phải "đích là script hash THẬT".
            ...
            có lớp mã nào thay được bước đó (`registry.ak:97-104`).

M-ADDR    util.has_no_stake_part(reg_out.address)   (v2.1)               (`:294`)
          ⇒ cùng lỗ với U-ADDR, và ĐẮT HƠN ở đây: hồ sơ vừa rời quyền tài phán này, nên nếu nó hạ cánh
            ...
            TRỐNG, và không đường nào ở đây gọi nó về được nữa (`registry.ak:290-293`).

M-ID      platform.identity_preserved(entry_in, entry_out)  — SÁU trường (`:300`)
M-STATUS  entry_out.status == entry_in.status                            (`:302`)
          ⇒ chống Retire trá hình: di trú KHÔNG phải cửa gỡ niêm yết.
M-VER     new_spec_version > entry_in.spec_version                       (`:304`)
            ∧ entry_out.spec_version == new_spec_version                 (`:305`)
          ⇒ chỉ TIẾN, và datum ra phải khai đúng phiên bản đã tuyên trong redeemer.
M-MUT     platform.mutable_fields_valid(entry_out)                       (`:308`)
          ⇒ thiếu dòng này, nhánh CỨU tự ghi ra hồ sơ khoá chết (gov rỗng / cut_bps ngoài dải) ở
            registry mới — nhánh cứu tự tạo lại đúng cái nó sinh ra để cứu (`registry.ak:115-120`).

M-GOVSELF-OUT entry_out.governance_ref != new_registry_hash              (`:314`)
            ∧ entry_out.governance_ref != own_hash                       (`:315`)
...
M-GOV2    entry_out.governance_ref == entry_in.governance_ref            (`:320-323`)
            ∨ governance_consented(tx, entry_out.governance_ref)
          ⇒ cùng luật U-GOV2. Thiếu nó, Migrate là CỬA SAU đi vòng qua U-GOV2: M-GOV chỉ đòi ref CŨ,
            ...
            PASS trước bản vá chính là lỗ đó (`registry.ak:124-132`).

M-SHAPE   shape_custodial(entry_in) == shape_custodial(entry_out)        (`:325`)
M-NFT     beacon NFT có ở cả vào lẫn ra                                  (`:328-329`)
M-VALUE   util.value_not_drained(reg_in.value, reg_out.value)            (`:332`)

KHÔNG ép U-TERMINAL ở nhánh này — CỐ Ý.
else fail — KHÔNG xoá hồ sơ (retire = status, không spend-burn — PK5).
```

> **Vì sao nhánh di trú cố ý KHÔNG áp U-TERMINAL** (`registry.ak:135-137`): nếu áp cả hai nhánh thì mỗi
...
> ⚠ **GIẢ ĐỊNH LOAD-BEARING — MẤT KHOÁ AUTHORITY ⇒ TOÀN SỔ ĐÔNG CỨNG** (`registry.ak:139-143`). U-SIG và
...
> ⚠ **GIẢ ĐỊNH LOAD-BEARING — v1 "hai bên" THỰC CHẤT LÀ MỘT BÊN** (`util.ak:226-229`).
...
| Bên đó lấy thông tin ở đâu | self-ref NFT: chọn ô ra bằng "token tôi vừa mint" | đọc từ datum: `entry_in.beacon_policy` (`registry.ak:172`) |
| Biên dịch trước | `registry_beacon(authority)` | `registry(authority)` (`registry.ak:161`) |
| Biên dịch sau | `registry(authority, beacon_policy)` | `registry_beacon(authority, registry_hash)` (`registry_beacon.ak:87`) |
| Cửa đúc ép được đích không | **KHÔNG** — chỉ ép `!is_vk` (Script bất kỳ) | **CÓ** — `is_at_script(entry_out.address, registry_hash)` (`registry_beacon.ak:106`) |
...
Self-ref NFT KHÔNG bị bỏ — `output_with_token` vẫn là cách CHỌN ô ra (`registry_beacon.ak:105`,
`util.ak:116-124`). v2 chỉ ép thêm một dòng lên ô đã chọn. Gương mẫu `custody_seed` Treasury
...
(`registry_beacon.ak:125`), nên hồ sơ hợp lệ không nói dối được. Nhưng một UTxO **TỰ DỰNG** đặt thẳng ở
...
> `registry.ak:237-243` nhận `pure_revive ∧ governance_consented(entry_in.governance_ref)`. Trong ô tự
...
| **PK4 identity SÁU trường bất biến** | U-ID + M-ID (`platform.ak:218-220`) | **first-principles:** đổi định danh = platform khác → phải đăng ký mới, không lén qua update. v2 thêm `beacon_policy` vào nhóm. |
...
| **PK6 governance_ref RIÊNG/platform** | R-WF (`len == 28` — `platform.ak:209`) + R-GOVLIVE + checklist onboard | **bền vững (tách quyền):** #1B Treasury **ĐÓNG** (F10 — spec_hash gồm instance_id); dùng chung KHÔNG còn replay chéo. PK6 nay là **khuyến nghị tách quyền release** (blast-radius nhỏ), không bắt buộc bởi replay. Governance commit đúng instance_id khi tạo proposal. |
...
- **~~GAP-5 (audit #3) — hồ sơ lạc chỗ ở Script lạ~~ — ĐÓNG bằng R-OUT-1 v2 + R-ADDR v2.1.** Mô tả cũ
  ...
  `Script(registry_hash)` (`registry_beacon.ak:106`) rồi ép tiếp biến thể enterprise
  (`registry_beacon.ak:115`) — đảo chiều tham số là thứ làm được việc đó (§5). Cờ `foreignScript` của
...
- **~~GAP-6 — `governance_ref` riêng vì replay-chéo #1B~~ — KHÔNG còn là gap an toàn (vá lần 2 F10).**
  ...
  `len(governance_ref) == 28` (`platform.ak:209`), cộng R-GOVLIVE bắt cổng quản trị chạy thật lúc đăng ký.
