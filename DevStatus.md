...
| **Dòng giá on-chain: 15 mã còn sống, trần `max_op_prices` cho CẢ HỆ** — `PriceParam` là một beacon toàn hệ, trần @ `ConsumeMAGIC/onchain/lib/magiclamp/consume/pricing.ak`; đo 2026-09-24 trên `MAGIC@443859ee` = 16 ⇒ còn 1 (`Specs/Resource-Dictionary.md` RD-6). Cấp **số** không có trần; cấp **dòng giá** thì có | Trần **sẽ giảm**: ConsumeMAGIC tách `demand_mult` theo từng `op_type` (`ConsumeMAGIC/CONTRACT.md` ▸ `CC-LOAD-COUNT-UNIT`), mỗi dòng nặng thêm, số dòng chứa được đo lại khi trường mới có mã. Nâng trần đã bị loại theo đường cong MEM (32 dòng). Hình dạng cho nhiều bảng thuộc ConsumeMAGIC. Ràng buộc đang áp: cấp số không kèm cam kết dòng giá; báo ConsumeMAGIC trước khi cấp mã cần dòng giá mới | `command grep -n 'max_op_prices' ../MAGIC/ConsumeMAGIC/onchain/lib/magiclamp/consume/pricing.ak` |
