...
| **Dòng giá on-chain còn 2/16 cho CẢ HỆ** — `PriceParam` là một beacon toàn hệ, trần `max_op_prices` @ `ConsumeMAGIC/onchain/lib/magiclamp/consume/pricing.ak`; 14 mã còn sống (`Specs/Resource-Dictionary.md` RD-6). Cấp **số** không có trần; cấp **dòng giá** thì có | Mỗi mã mới tiêu một trong hai dòng cuối. Nâng trần phải đo lại MEM của `required_for`, không suy ra | `command grep -n 'max_op_prices' ../MAGIC/ConsumeMAGIC/onchain/lib/magiclamp/consume/pricing.ak` |
