...
| **Dòng giá on-chain còn 1/16 cho CẢ HỆ** — `PriceParam` là một beacon toàn hệ, trần `max_op_prices` @ `ConsumeMAGIC/onchain/lib/magiclamp/consume/pricing.ak`; 15 mã còn sống sau khi cấp mã 19 (`Specs/Resource-Dictionary.md` RD-6). Cấp **số** không có trần; cấp **dòng giá** thì có | Mã kế tiếp tiêu dòng cuối cùng. Nâng trần phải đo lại MEM của `required_for`, không suy ra | `command grep -n 'max_op_prices' ../MAGIC/ConsumeMAGIC/onchain/lib/magiclamp/consume/pricing.ak` |
