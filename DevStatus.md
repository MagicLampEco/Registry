...
| Kiểm thử on-chain (`registry_test.ak`, `registry_beacon_test.ak`) | `đã merge (commit c63372e)`, nhưng ⚠ **độ phủ THẬT chưa bằng số bài** — xem dòng 16-chốt-hở ở mục *Việc còn treo* | cùng PR | `cd onchain && aiken check` — và ĐỘ PHỦ thì phải đo bằng đột biến, không đọc từ số bài xanh | mốc M2, M3 của Exec-Spec | 2026-09-05 |
...
|---|---|---|
| ~~16 trong 52 chốt on-chain không có bài kiểm nào canh riêng~~ — **VÁ 2026-09-05**, đo lại còn **5**, và 5 cái đó không phải lỗ (xem dòng dưới). Nền: gỡ hẳn từng `expect` rồi chạy trọn bộ ⇒ 16 chốt gỡ ra mà **0 bài đỏ**, gồm `R-SIG` (authority ký lúc đúc), `R-MINT-1`, `R-NAME`, `R-POLICY`, `R-GOVSELF`, `S-GOVSELF`, `U-NFT`/`M-NFT`, `U-SHAPE`/`M-SHAPE` | Gốc chung ở 11/16: bài âm tính lệch ca hợp lệ ở **hai** chỗ, chỗ thứ hai thêm vào vì "đằng nào cũng đỏ trước đó" ⇒ bài chết ở chốt sớm hơn và **mang tên một chốt nó không canh**. Bằng chứng: gỡ `R-SIG` rồi `aiken check -m register_no_authority_sig` → **vẫn PASS** | `node <scratchpad>/dot-bien.mjs` — gỡ từng chốt, chạy trọn bộ, in chốt nào còn hở, rồi khôi phục. ~10 phút |
| 🟡 **5 chốt còn lại là trạng thái THỨ BA, không phải "còn 5 lỗ".** Bốn cái đầu là hai CẶP `U-NFT` và `M-NFT` trong `registry.ak` — mỗi cặp là hai dòng **tương đương logic**, vì `identity_preserved` và `value_not_drained` đã ép chúng bằng nhau ở hai đầu. Cái thứ năm là `R-MINT-1` vế `dict.size`, bị **bao hàm** bởi phép gán ngay dòng sau nó: khớp `[Pair(…)]` chỉ thành công khi dict có đúng một phần tử | Hành vi **vẫn được ghim**, chỉ ghim theo CẶP chứ không theo DÒNG. Đo: gỡ riêng `:226` → 0 đỏ; gỡ riêng `:227` → 0 đỏ; gỡ **cả cặp** → **2 đỏ** | gỡ cặp bằng tay rồi `aiken check`; hoặc đọc chú thích tại chỗ trong hai bài `update_no_beacon_nft_in_and_out` và `migrate_no_beacon_nft_in_and_out` |
| **6 chốt KHÔNG ĐO ĐƯỢC bằng phép đột biến đang có** — bốn khối `expect or { … }` và hai biểu thức nhiều dòng: thay cả khối bằng `expect True` làm mất binding bên trong ⇒ không biên dịch được ⇒ phép đo im lặng | Đây là trạng thái **mù**, không phải trạng thái sạch, nên nó phải kêu to hơn "lệch". Cần một phép đột biến khác cho khối nhiều dòng (đảo vế thay vì gỡ khối) | trong kết quả JSON của script, các dòng `baiDo: -1` |
